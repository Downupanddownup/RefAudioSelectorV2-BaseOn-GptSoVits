...
class DatabaseConfig(metaclass=SingletonMeta):
    ...
    def update_db_path(self, role: Role):
        self.role = role
        self.db_path = f'{self.get_role_work_dir()}\\ref_audio_selector.db'
    ...
    def get_role_work_dir(self) -> str:
        work_dir = f'{self.get_slave_dir()}\\{self.role.category}\\{self.role.name}'
        if not os.path.exists(work_dir):
            os.makedirs(work_dir)
        return work_dir

    def get_role_model_dir(self) -> str:
        model_dir = f'{self.get_role_work_dir()}\\model'
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        return model_dir

    def get_slave_dir(self) -> str:
